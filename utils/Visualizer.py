...
class SimulationAnalyzer:
    def __init__(self, simulation_folder):
        ...
        if not os.path.exists(simulation_folder):
            self.simulation_dir = os.path.join("simulation_data", simulation_folder)
        else:
            self.simulation_dir = simulation_folder

        if not os.path.exists(self.simulation_dir):
            raise FileNotFoundError(f"Simulation directory not found: {self.simulation_dir}")
...
if __name__ == "__main__":
    ...
    analyzer = SimulationAnalyzer("particle_filter_20250620_001614_p_1000")
